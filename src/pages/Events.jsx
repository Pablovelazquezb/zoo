...
export default function Events() {
    ...
    async function fetchResources() {
        // Use explicit foreign key for departments to avoid circular dependency ambiguity
        const { data: staffData } = await supabase.from('employees').select('employee_id, first_name, last_name, departments!employees_dept_id_fkey(dept_name)');
        const { data: animalData } = await supabase.from('animals').select('animal_id, name, species_common_name');
                ...
                <div className="grid-cards">
                    {events.map(event => (
                        <div
                            ...
                            style={{ padding: '20px', position: 'relative', overflow: 'hidden', cursor: 'pointer', transition: 'transform 0.2s' }}
                ...
                <div style={{
                    ...
                    <div className="glass-panel" style={{ width: '600px', maxWidth: '90vw', maxHeight: '80vh', overflowY: 'auto', padding: '30px', background: '#0f172a', border: '1px solid var(--glass-border)' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
                            ...
                            <button onClick={() => setSelectedEvent(null)} style={{ background: 'none', border: 'none', color: 'white', cursor: 'pointer' }}><X /></button>
                        </div>

                        <div style={{ marginBottom: '30px', paddingBottom: '20px', borderBottom: '1px solid rgba(255,255,255,0.1)' }}>
                            <h3 style={{ color: 'var(--color-primary)' }}>{new Date(selectedEvent.event_date).toDateString()}</h3>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px', marginBottom: '30px' }}>
                            ...
                            <div style={{ background: 'rgba(255,255,255,0.05)', padding: '15px', borderRadius: '10px' }}>
                                <h4 style={{ display: 'flex', alignItems: 'center', gap: '10px', marginTop: 0 }}><User size={16} /> Staff Assigned</h4>
                                {assignments.filter(a => a.employees).map(a => (
                                    <div key={a.assignment_id} style={{ fontSize: '14px', marginBottom: '5px' }}>
                                        ...
                                        <span style={{ fontSize: '10px', color: 'var(--color-text-muted)', marginLeft: '5px' }}>({a.employees.departments?.dept_name})</span>
                                    </div>
                                ))}
                                {assignments.filter(a => a.employees).length === 0 && <p style={{ fontSize: '12px', color: 'gray' }}>No staff assigned.</p>}
                            </div>

                            {/* Animal List */}
                            <div style={{ background: 'rgba(255,255,255,0.05)', padding: '15px', borderRadius: '10px' }}>
                                <h4 style={{ display: 'flex', alignItems: 'center', gap: '10px', marginTop: 0 }}><Cat size={16} /> Animals Featured</h4>
                                {assignments.filter(a => a.animals).map(a => (
                                    <div key={a.assignment_id} style={{ fontSize: '14px', marginBottom: '5px' }}>
                                        {a.animals.name}
                                        <span style={{ fontSize: '10px', color: 'var(--color-text-muted)', marginLeft: '5px' }}>({a.animals.species_common_name})</span>
                                    </div>
                                ))}
                                {assignments.filter(a => a.animals).length === 0 && <p style={{ fontSize: '12px', color: 'gray' }}>No animals assigned.</p>}
                            </div>
                        </div>

                        {/* Assign Form */}
                        <form onSubmit={handleAssign} style={{ background: 'rgba(255,255,255,0.05)', padding: '20px', borderRadius: '10px' }}>
                            <h4 style={{ marginTop: 0 }}>Assign New Resource</h4>
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                                <select
                                    className="glass-input"
                                    value={assignmentForm.employee_id}
                                    onChange={e => setAssignmentForm({ ...assignmentForm, employee_id: e.target.value, animal_id: '' })}
                                >
                                    <option value="">Select Staff...</option>
                                    {staff.map(s => <option key={s.employee_id} value={s.employee_id}>{s.first_name} {s.last_name}</option>)}
                                </select>
                                <span style={{ textAlign: 'center', alignSelf: 'center' }}>OR</span>
                                <select
                                    className="glass-input"
                                    value={assignmentForm.animal_id}
                                    onChange={e => setAssignmentForm({ ...assignmentForm, animal_id: e.target.value, employee_id: '' })}
                                >
                                    <option value="">Select Animal...</option>
                                    {animals.map(a => <option key={a.animal_id} value={a.animal_id}>{a.name} ({a.species_common_name})</option>)}
                                </select>
                            </div>
                            <button type="submit" disabled={!assignmentForm.employee_id && !assignmentForm.animal_id} className="glass-button" style={{ width: '100%', marginTop: '15px' }}>
                                <Plus size={16} style={{ marginRight: '5px' }} /> Assign
                            </button>
                        </form>
                    </div>
                </div>
